...
-export([make_tables/0, how_many_trigrams/0, make_ets_set/0, make_ets_ordered_set/0, make_mod_set/0,
    timer_tests/0, lookup_all_set/2, lookup_all_ets/2,
    is_word/2, open/0, close/1]).

make_tables() ->
    {Micro, N} = timer:tc(?MODULE, how_many_trigrams, []),
    io:format("Counting - No of trigrams = ~p time/trigram=~p~n", [N, Micro/N]),
    {Micro1, Ntri} = timer:tc(?MODULE, make_ets_ordered_set, []),
    FileSize1 = filelib:file_size("trigramsOS.tab"),
    io:format("Ets ordered set size = ~p time/trigram=~p~n", [FileSize1/Ntri, Micro1/N]),
    {Micro2, _} = timer:tc(?MODULE, make_ets_set, []),
    FileSize2 = filelib:file_size("trigramsS.tab"),
    io:format("Ets set size = ~p time/trigram=~p~n", [FileSize2/Ntri, Micro2/N]),
    {Micro3, _} = timer:tc(?MODULE, make_mod_set, []),
    FileSize3 = filelib:file_size("trigrams.set"),
    io:format("Set size = ~p time/trigram=~p~n", [FileSize3/Ntri, Micro3/N]).


for_each_trigram_in_the_english_language(F, A0) ->
    {ok, Bin0} = file:read_file("words.tgz"),
